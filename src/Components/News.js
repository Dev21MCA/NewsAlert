...
const News = ({ setProgress }) => {
  ...
  const updateNews = async (currentKeyword) => {
    setProgress(10);
    const url = `https://newsapi.org/v2/everything?q=${currentKeyword}+q=Stocks&sortBy=publishedAt&apiKey=61f2ecc2cba6494b9706ebb842729459&pageSize=20&page=${page}`;
    setLoading(true);
  ...
  return (
    <div className="container my-3">
      <div>
        {/* yaha se navbar start hai */}
        <div id="new-navbar">
        <nav style={{height:"4rem"}} id="navbar-header"  className="navbar fixed-top bg-dark border-bottom border-bottom-dark"
          data-bs-theme="dark" >
        ...
        style={{marginTop:"0rem",marginLeft:"70rem",position:"absolute"}}
        id="wishlist-keyword"
          type="text"
        ...
        />
        <button className="btn btn-outline-success"  id="wishlist-button" onClick={handleSearch}>+ WatchList</button>
        </nav>
        </div>
      ...
      <div>
        <p  id="user-keyword-titles">User Keywords: {userKeywords.join(", ")}</p>
      </div>
